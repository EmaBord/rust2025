...
- La address enviada por discord: `VwJHxi1vTNdnJ2KXfQ7tWL8ERiUGv9bwhmf5RDWKGySQmsy` es distinta a la verdadera adress del contrato (presente en el README del repositorio del grupo) `XDHDTFonKyVQnTZaB9TpMcfTKWkuuL9TaDR4mBz5ebVWnYV`.
- No existe manera de modificar los roles del usuario una vez creado, es necesario que este pueda cambiar de `Vendedor` o `Comprador` a `Ambos`.
- Aunque existe una manera de visualizar ordenes y productos por el ID de estos. No existe una manera de listar los productos del vendedor ni las ordenes de compra del comprador.
- A los productos publicados les falta la descripción y categoría.
- La documentación está clara, completa y bien documentada. Sería recomendable reducir la cantidad de comentarios en funciones internas, la documentación de las funciones con `#[ink(message)]` son un buen ejemplo de como deberían documentarse estas.
