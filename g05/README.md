...
- [ Cumple ] Permite modificar roles de usuario luego del registro.
...
- [ Suficiente [2] ] El usuario puede visualizar sus propios productos publicados.

### Compra y gestión de órdenes
- [ Cumple ] Solo usuarios con rol `Comprador` pueden crear órdenes de compra.
- [ Cumple [3] ] Al comprar, se crea la orden y se descuenta el stock correctamente.
- [ Cumple [4] ] La orden puede tener estado: `pendiente`, `enviado`, `recibido`.
...
- [ Cumple ] El contrato desplegado en testnet es **funcional** y permite interactuar con todas las funcionalidades requeridas.

---

## 3. Testing y calidad del código

- [ Suficiente [5] ] Existe una suite de tests automatizados que cubre **≥ 85%** del código del contrato.
- [ Suficiente [6] ] El código está bien estructurado y comentado según lo visto en clase.
- [ Cumple [7] ] Incluye documentación técnica clara para las funcionalidades implementadas.

#### Set mínimo de pruebas obligatorio:
- [ Suficiente [8] ] Test de registro de usuario con cada rol posible.
- [ Cumple ] Test de publicación de producto.
- [ Cumple [9] ] Test de compra de producto y generación de orden.
- [ Cumple [10] ] Test de cambio de estado de la orden (`pendiente` → `enviado` → `recibido`).
- [ Cumple ] Test de validación de permisos (solo quien corresponde puede ejecutar cada acción).
- [ Cumple [11] ] Test de errores esperados (ej: intentar comprar sin stock, cambiar estado sin permisos, etc.).
...
- [ Cumple ] Cambiar el rol de un usuario y chequear que el cambio se refleje en el contrato.

### Publicación de productos
- [ Cumple ] Como `Vendedor`, publicar al menos un producto con todos los datos requeridos.
- [ Suficiente [12] ] Verificar que el producto figure en la lista de productos del vendedor.

### Compra y órdenes
- [ Cumple ] Como `Comprador`, realizar la compra de un producto disponible.
- [ Cumple ] Chequear que se descuente correctamente el stock tras la compra.
- [ Cumple ] Verificar que la orden queda en estado `pendiente`.
...
- [ Suficiente [13] ] Intentar que alguien que no sea el vendedor cambie el estado a `enviado` (debe fallar).
- [ Suficiente [14] ] Intentar que alguien que no sea el comprador cambie el estado a `recibido` (debe fallar).
- [ Cumple ] Intentar publicar un producto sin ser `Vendedor` (debe fallar).

### General
- [ Cumple [15] ] Confirmar que los cambios de estado y acciones se reflejan correctamente en el almacenamiento y pueden ser consultados vía RPC.
