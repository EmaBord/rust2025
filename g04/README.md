...
- La implementación de `modificar_rol()` permite a un usuario eliminarse roles. Un usuario `Comprador` o `Vendedor` debería poder únicamente pasar a tener el rol `Ambos`, cualquier otro cambio de rol debería ser rechazado.
- No existe manera de listar los productos publicados del vendedor.
